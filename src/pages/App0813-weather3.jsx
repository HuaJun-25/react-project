	...
	const CardBody = ({item, index, pop}) => {
		return (
			<>
				...
				<p className="pop">
					<IoMdUmbrella />{pop.time[index].parameter.parameterName}%
				</p>
			</>
		)
	}

	function Card({ citys, title, wx, pop }) {
		return (
			<div >
							...
							wx.time.map((item, index) => {
								return (
									<div className="card-body" key={index}>
										<CardBody item={item} citys={citys} index={index} pop={pop}/>
									</div>
								)
							})
						}
		...
		<>
			<div className="wrap">
...

				{
					citys.map((citys) => {
						return (
							<Card citys={citys} key={citys.locationName} title={citys.locationName} wx={citys.weatherElement[0]} pop={citys.weatherElement[4]} />
						)
					})
				}
			</div>
